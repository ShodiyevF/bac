...
const clientPUTNumber2Model = async (user_id, { company_id, client_id, client_phone_number_second }) => {
    try {
        ...
        if (checkuser.rows.length) {
            const c = (await uniqRow('select * from clients where company_id = $1 and client_id = $2', company_id, client_id)).rows
            if (c[0]) {
                await uniqRow('update clients set client_phone_number_second = $1 where client_id = $2', client_phone_number_second, client_id)
                return 200
            } else {
                return 404
            }
...
const clientPUTAboutModel = async (user_id, { company_id, client_id, client_about }) => {
    try {
        ...
        if (checkuser.rows.length) {
            const c = (await uniqRow('select * from clients where company_id = $1 and client_id = $2', company_id, client_id)).rows
            if (c[0]) {
                await uniqRow('update clients set client_about = $1 where client_id = $2', client_about, client_id)
                return 200
            } else {
                return 404
            }
